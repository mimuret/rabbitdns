...
type Entry struct {
	...
	RRtype   uint16
	config   *Config
	endpoint resource_interface.Endpoint
...
func NewEntry(serviceName string, mon *Config, e resource_interface.Endpoint) *Entry {
...
func (e *Entry) getStatus() bool {
	return e.status
}
func (e *Entry) initStatus(status bool) {
	e.status = status
	e.endpoint.SetStatus(status)
	e.ngCounter = 0
	e.okCounter = 0
...
func (e *Entry) setStatus(lastStatus bool) {
	if e.status {
		if lastStatus {
			e.okCounter++
