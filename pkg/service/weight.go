...
func (we *Weight) GetEndpoints(req *GetEndpointsRequest) ([]dns.RR, error) {
	...
	for name, value := range we.Values {
		next_rr, err = value.Next.GetEndpoints(req)
		if err == nil {
			sumWeight += uint32(value.Weight)
			rrs[name] = next_rr
		}
	}
	if sumWeight == 0 {
	...
	var name string
	mod := rand.Uint32() % sumWeight
	for name, next_rr = range rrs {
		if mod <= uint32(we.Values[name].Weight) {
			break
		}
		mod -= uint32(we.Values[name].Weight)
	}
