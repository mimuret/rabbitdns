...
func NewEndpoint(config *Config, path string, v *viper.Viper) (Service, error) {
	...
	endpoint := &Endpoint{RRType: config.RRType,
		Value:  value,
		RR:     rr,
		path:   path,
		status: true,
...
func (e *Endpoint) GetEndpoints(req *GetEndpointsRequest) ([]dns.RR, error) {
	if e.status {
...
func (e *Endpoint) GetRRtype() uint16 {
	return e.RRType
}

func (e *Endpoint) SetStatus(status bool) {
	e.status = status
}
